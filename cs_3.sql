...
SELECT DATE_PART('month' , start_date) AS date ,
COUNT(*) AS nm_of_monthly_disti
...
SELECT p.plan_name,count(*) FROM foodie_fi.subscriptions s
...
WITH ranking AS(SELECT plan_id , customer_id ,
			ROW_NUMBER() OVER (partition by customer_id) AS rank
		    FROM foodie_fi.subscriptions ) 

SELECT COUNT(*) AS count_of_cust_after_trial , 
ROUND(100* COUNT(*) / (SELECT COUNT(DISTINCT customer_id) FROM foodie_fi.subscriptions),1) AS churned_percen
FROM ranking 
WHERE rank = 2 AND plan_id = 4;

-- What is the number and percentage of customer plans after their initial free trial?
WITH np AS (
			SELECT customer_id , plan_id ,LEAD(plan_id,1) OVER(partition by customer_id) AS next_plan
			FROM foodie_fi.subscriptions
			)
SELECT next_plan, COUNT(*),ROUND(100* COUNT(*)/ (SELECT COUNT(DISTINCT customer_id) 
    FROM foodie_fi.subscriptions)) AS percentage 
	FROM np 
...
c_breakdown AS (
SELECT plan_id, 
    COUNT(DISTINCT customer_id) AS customers FROM np
...
WITH tp AS(SELECT customer_id, start_date AS join_date 
		   FROM foodie_fi.subscriptions s
		  WHERE s.plan_id = 0),
ap AS(SELECT  customer_id,start_date AS annual_date
		   FROM foodie_fi.subscriptions s
		  WHERE s.plan_id = 3)

SELECT ROUND(AVG(annual_date - join_date),1) 
FROM tp t
JOIN ap a
...
SELECT customer_id, plan_id, start_date,
    LEAD(plan_id, 1, 0) OVER( PARTITION BY customer_id ORDER BY plan_id) as next_plan
FROM foodie_fi.subscriptions)
