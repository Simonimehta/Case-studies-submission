...
GROUP BY region_id
ORDER BY region_id;
-- SELECT COUNT(node_id ) FROM data_bank.customer_nodes
-- Where region_id = 1

-- How many customers are allocated to each region?
SELECT region_id,COUNT(DISTINCT customer_id) FROM data_bank.customer_nodes
GROUP BY region_id
ORDER BY region_id;
...
WITH cte AS(
SELECT  node_id, customer_id , ("end_date"::DATE - "start_date" ::DATE) AS d
FROM data_bank.customer_nodes
WHERE end_date != '9999-12-31'
GROUP BY customer_id , node_id ,end_date ,start_date),

cte_1 AS(
SELECT node_id, customer_id  , SUM(d) AS s
	FROM cte
	GROUP BY customer_id , node_id
)
SELECT ROUND(AVG(s),3)
FROM cte_1;
...
-- SELECT node_id , ROUND(AVG(d)) FROM cte
-- GROUP BY node_id
-- ORDER BY node_id;
...
WITH node_diff AS (
SELECT customer_id, node_id, start_date, end_date, region_id ,(end_date - start_date) AS diff
FROM data_bank.customer_nodes
WHERE end_date != '9999-12-31'
GROUP BY customer_id, node_id, start_date,end_date,region_id
)

SELECT region_id,
percentile_disc(0.5) WITHIN GROUP (ORDER BY diff) AS median,
percentile_disc(0.8) WITHIN GROUP (ORDER BY diff) AS median_80,
percentile_disc(0.95) WITHIN GROUP (ORDER BY diff) AS median_95
FROM node_diff
GROUP BY region_id;


-- SELECT PERCENTILE_DISC(0.80) WITHIN GROUP(ORDER BY region_id) FROM  data_bank.customer_nodes
-- GROUP BY region_id;

-- --                           B. Customer Transactions
-- What is the unique count and total amount for each transaction type?
SELECT txn_type , COUNT(DISTINCT txn_amount) AS unqiqu_count, 
...
WITH cte AS
(SELECT customer_id , COUNT(txn_type) AS coun , txn_amount AS amount 
FROM data_bank.customer_transactions
WHERE txn_type = 'deposit'
GROUP BY customer_id,txn_amount),
cte_1 AS
(SELECT customer_id , SUM(coun) AS s , SUM(amount) AS a 
FROM cte
 GROUP BY customer_id
)
SELECT ROUND(AVG(s),2) , ROUND(AVG(a),2) 
FROM cte_1;
...
TO_CHAR(txn_date , 'MONTH') AS  month_name FROM data_bank.customer_transactions
GROUP BY customer_id ,month_name)

SELECT COUNT(distinct customer_id) AS customer_count,month_name
FROM cte
WHERE deposit > 1 AND (purchase >= 1 OR withdrawal >= 1 )
GROUP BY month_name
ORDER BY month_name;
-- EXTRACT(month from TXN_DATE) AS month,- month nm

-- What is the closing balance for each customer at the end of the month?
-- SELECT DISTINCT(customer_id) , 
-- (date_trunc('MONTH',txn_date) + INTERVAL '1 MONTH - 1 day') AS end_day_of_month, 
...
SUM(txn_amount) AS closing_balance
FROM data_bank.customer_transactions
GROUP BY customer_id ,end_day_of_month, month_name
ORDER BY customer_id;
